// Edit file path===>  /Users/growthustec/Desktop/Apps/retyrn/node_modules/react-native-maps-directions/src/MapViewDirections.js
...
class MapViewDirections extends Component {
	...
	fetchAndRenderRoute = (props) => {
		...
		Promise.all(
			...
			.then((results) => {
				...
				const totalDistance = allRoutes.reduce(
	...
	fetchRoute(
		...
		if (typeof directionsServiceBaseUrl === "string") {
			url += `?origin=${origin}&destination=${destination}&key=${apikey}&mode=${mode.toLowerCase()}&language=${language}&region=${region}&alternatives=${alternatives}`;
			if (timePrecision) {
			...
			.then((json) => {
				...
				if (json.routes.length) {
					return json.routes.map((route) => ({
						distance:
							route.legs.reduce(
								(carry, curr) => carry + curr.distance.value,
								0
							) / 1000,
						duration:
							route.legs.reduce(
						...
						coordinates:
							precision === "low"
								...
								: route.legs.reduce(
										(carry, curr) => [...carry, ...this.decode(curr.steps)],
										[]
								  ),
						fare: route.fare,
	...
	render() {
		...
		return (
			<>
				{routes.map((route, index) => (
					<Polyline key={index} coordinates={route.coordinates} {...props} />
				))}
			</>
		);
	}
