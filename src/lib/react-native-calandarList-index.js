// Edit file path===>  /Users/growthustec/Desktop/Apps/retyrn/node_modules/react-native-calendars/src/calendar-list/index.js
...
const CalendarList = (props, ref) => {
	...
	const shouldUseAndroidRTLFix = useMemo(
		() => constants.isAndroidRTL && horizontal,
		[horizontal]
	);
	...
	const scrollToDay = (date, offset, animated) => {
		...
		if (scrollAmount !== 0) {
			...
			list?.current?.scrollToOffset({ offset: scrollOffset, animated });
		}
	};
		...
		({ viewableItems }) => {
			...
			if (shouldUseAndroidRTLFix) {
				const centerIndex = items.findIndex((item) =>
					isEqual(parseDate(current), item)
				);
				const adjustedOffset =
					centerIndex -
					items.findIndex((item) => isEqual(newVisibleMonth, item));
